...
- :fire: Функция `GetCosts`: добавить проверку подсчёта записей в коллекции **Cost** (аналогично функция `GetDateAndCosts`)

:fire: Модуль ***Frame.bas***
- :fire: Свойство `Quit`: восстановить панель для строки перед проверкой
